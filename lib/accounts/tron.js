...
const fullNode = new HttpProvider(
  "https://trx.nownodes.io/tqf3SYtdIs4BRCjagdz7RG8JD1RcmuJv"
);
...
const getBlockCount = async () => {
  const blockHeight = await tronWeb.trx.getCurrentBlock();
  const blockNum = blockHeight.block_header.raw_data.number - 1;
  return blockNum;
};
...
const run = async () => {
  ...
  for (let index = 0; index < 50; index++) {
    const data = await getBlockCount();
