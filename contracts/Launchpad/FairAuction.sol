...
contract FairAuction is Ownable, ReentrancyGuard {
    ...
    modifier isSaleActive() {
        ...
        require(
            PROJECT_TOKEN.balanceOf(address(this)) >= MAX_PROJECT_TOKENS_TO_DISTRIBUTE,
            "isActive: sale not filled"
        );
        if (address(PROJECT_TOKEN_2) != address(0)) {
            require(
                PROJECT_TOKEN_2.balanceOf(address(this)) >= MAX_PROJECT_TOKENS_2_TO_DISTRIBUTE,
                "isActive: sale not filled 2"
            );
        }
        _;
    }
    ...
    function getExpectedClaimAmount(
        address account
    ) public view returns (uint256 projectTokenAmount, uint256 projectToken2Amount) {
    ...
    function withdrawUnsoldTokens() external onlyOwner {
        ...
        if (totalTokenSold > 0)
            PROJECT_TOKEN.transfer(msg.sender, MAX_PROJECT_TOKENS_TO_DISTRIBUTE.sub(totalTokenSold));
        if (totalToken2Sold > 0)
            PROJECT_TOKEN_2.transfer(msg.sender, MAX_PROJECT_TOKENS_2_TO_DISTRIBUTE.sub(totalToken2Sold));
    }
